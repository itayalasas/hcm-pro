...
import { FileText, Plus, Edit2, Trash2, Download, CheckCircle, Search } from 'lucide-react';
...
export default function Forms() {
  ...
  const loadDocuments = async () => {
    try {
      ...
      showToast(error.message || 'Error al cargar formularios', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(error.message || 'Error al guardar formulario', 'error');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      showToast(error.message || 'Error al eliminar formulario', 'error');
    } finally {
  ...
  const handlePublish = async (doc: Document) => {
    try {
      ...
      if (error) throw error;
      showToast('Formulario publicado exitosamente', 'success');
      loadDocuments();
    } catch (error: any) {
      console.error('Error publishing form:', error);
      showToast(error.message || 'Error al publicar formulario', 'error');
    }
  };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Formularios</h1>
          <p className="text-slate-600">Gestión de formularios y plantillas</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium text-slate-600">Total Formularios</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium text-slate-600">Descargas (30d)</p>
        ...
        <div className="overflow-x-auto">
          {filteredDocuments.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-slate-900 mb-2">No hay formularios</h3>
              <p className="text-slate-600 mb-4">Comienza creando tu primer formulario</p>
              <Button onClick={handleCreate}>
                <Plus className="w-4 h-4 mr-2" />
                Crear Formulario
              </Button>
      ...
      <Modal
        ...
        title={selectedDocument ? 'Editar Formulario' : 'Nuevo Formulario'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input
            ...
            placeholder="formulario-solicitud-vacaciones.pdf"
          />

          <div className="flex gap-3 pt-4">
            <Button type="submit" className="flex-1">
              {selectedDocument ? 'Actualizar' : 'Crear'} Formulario
            </Button>
      ...
      <ConfirmDialog
        ...
        message="¿Está seguro de que desea eliminar este formulario? Esta acción no se puede deshacer."
      />
    </div>
  );
}
